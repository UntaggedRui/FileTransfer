...
int main(int argc, char *argv[])
{
    ...
    if (run_latency == 1)
    {
        ...
        pthread_create(&monitor_thead, NULL, monitor_bandwidth, NULL);
    }
...
void *monitor_bandwidth()
{
    ...
    {
        // Record the amount of data transmitted by each thread in one second separately.
        for ( threadId = 0; threadId < thrds; threadId++)
        {
            bandwidth[threadId][seconds] = 1.0 * read_persecond[threadId] / 1024 / 1024 / 1024;
            if (DEBUG)
            {
                printf("thread %d bandwidth is %lf Gbits/sec\n", threadId, bandwidth[threadId][seconds] * 8);
            }
            read_persecond[threadId] = 0;
        }

        /*  bandwidth[i] = 1.0 * read_persecond / 1024 / 1024 / 1024;
        printf("now bandwidth is %lf Gbits/sec\n", bandwidth[i] * 8); */
        seconds++;
